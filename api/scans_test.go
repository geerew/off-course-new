...
func TestScans_GetScans(t *testing.T) {
	...
	t.Run("200 (found)", func(t *testing.T) {
		...
		for _, item := range respData.Items {
			...
			require.Nil(t, json.Unmarshal(item, &scan))
			require.Nil(t, err)
	...
	t.Run("200 (expand)", func(t *testing.T) {
		...
		resp, err := f.Test(httptest.NewRequest(http.MethodGet, "/api/scans/?orderBy=created_at%20asc&expand=true", nil))
		assert.NoError(t, err)
		require.Equal(t, http.StatusOK, resp.StatusCode)
...
func TestScans_GetScan(t *testing.T) {
	...
	t.Run("200 (expand)", func(t *testing.T) {
		...
		resp, err := f.Test(httptest.NewRequest(http.MethodGet, "/api/scans/"+scans[2].ID+"?expand=true", nil))
		assert.NoError(t, err)
		require.Equal(t, http.StatusOK, resp.StatusCode)
