...
<Dialog.Root bind:open closeOnEscape={false} closeOnOutsideClick={false}>
	...
	>
		<div class="flex h-[min(calc(100vh-10rem),50rem)] flex-col">
			{#if open}
				<!-- Header -->
				<div
				...
				<div
					bind:this={body}
					class="flex min-h-[10rem] grow flex-col overflow-y-scroll"
					tabindex="-1"
				>
					{#if loadingDrives}
						<Loading />
					...
					{:else}
						<div class="flex flex-col">
							...
							{#each pathInfo.directories as dirInfo (dirInfo.path)}
								<div
									class="border-alt-1/40 flex h-14 flex-row items-center border-b last:border-none"
								>
									<!-- Path (left) -->
									<Button
										variant="ghost"
										disabled={loadingPath ||
											...
											dirInfo.isSelected}
										class="hover:bg-alt-1/20 h-full flex-grow justify-start rounded-none"
										on:click={async () => {
									...
									>
										...
										<Badge
											...
											class={cn(
												...
												dirInfo.classification !== PathClassification.Course && 'hidden'
											)}
										>
											Added
										</Badge>
										...
										<div class="flex h-full min-w-20 shrink-0 place-content-center items-center">
											...
											{:else}
												<Button
													...
													class="hover:bg-alt-1/20 group h-full w-14 shrink-0 place-content-center items-center rounded-none duration-200 disabled:opacity-100 sm:w-20"
													on:click={() => {
