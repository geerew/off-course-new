...
type Field struct {
	...
	// NotNull is a flag that indicates if the field is not null. It can be used during the create
	// or update operations to determine if the field should be set to null, when the field is
	// reflect.Zero()
	NotNull bool

	// SkipNull is a flag that indicates if the field should be skipped during the create or update
	// operations when the field is reflect.Zero(). This is useful when a field is not allowed to be
	// null in the database but there is a default value set in the database
	SkipNull bool
	...
	// 	- JOIN: the type of join (ex. left, right, ...)
	// 	- THIS: the column to join on in the current table
	// 	- ON: the foreign table.column to join on
